...
For the reusable toolkit, see [`nibbler/`](../../shared/nibbler/). For a guided walkthrough showing when and why each script is used, see [`apple-panic/Walkthrough.md`](../apple-panic/Walkthrough.md).
...
```
python apple-ii/scripts/<script_name>.py
```

Scripts use repo-relative default paths (derived from each script's location), so they work out of the box when the WOZ and DSK files are in `apple-ii/apple-panic/`. The 7 pipeline scripts also accept CLI arguments (e.g., `--woz`, `--output-dir`) to override defaults.

Script output goes to `apple-ii/apple-panic/output/` (gitignored), created automatically on first use.
...
| **render_sprites.py** | Render player, enemy, font, and platform sprites from runtime image as PNGs. | `apple-ii/apple-panic/assets/*.png` (6 images) |
| **woz_flux_image.py** | Render magnetic flux patterns as a top-down grayscale disk surface image. Wrapper around `nibbler flux`. | `apple-ii/apple-panic/flux_image.png` |
