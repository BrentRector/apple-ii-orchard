...
python -m nibbler decode "apple-ii/apple-panic/Apple Panic - Disk 1, Side A.woz" 0 --sector 0

# Disassemble the RWTS (the boot sector after relocation to $0200)
python apple-ii/scripts/disasm_rwts.py
```
...
```bash
python apple-ii/scripts/disasm_stage2.py
```
...
python apple-ii/scripts/decode_track0.py

# Verification
python apple-ii/scripts/verify_checksum.py
python apple-ii/scripts/compare_decoders.py
python apple-ii/scripts/check_wrap.py
```
...
python apple-ii/scripts/boot_emulate_full.py
```
...
```bash
python apple-ii/scripts/build_runtime.py
```
...
source ../../shared/toolchain/env.sh    # puts ca65/ld65 on PATH
python -m disasm6502 ApplePanic_runtime.bin --org $0000 --entry $4000 \
    --symbols ../../shared/symbols/apple2.json --output applepanic

# Full Apple-Panic-specific disassembly with labels, annotations, and data formatting
python apple-ii/scripts/disassemble.py
```
...
python apple-ii/scripts/woz_62verify.py

# Scan sector data consistency across revolutions
python apple-ii/scripts/scan_sector1.py
```
