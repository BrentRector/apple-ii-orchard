...
RAM_DISPATCH_TRAMPOLINE:
        ...
        NOP                              ; $0CB4  00
        NOP                              ; $0CB5  00
; Editor/LIST working flag (MBASIC equiv of GBASIC's $0C93; the low-RAM cells shift +$23 between the two builds). INPUT_PROMPT_SEP stores 0 or ';' here; INLIN_CR_FINISH tests it to choose auto-redisplay of a prefilled buffer vs normal line terminate; DETOKENIZE_LINE clears it as the inter-token spacing state. Was $0CB6. [RE]
DETOKENIZE_SPACE_FLAG:
        NOP                              ; $0CB6  00
; INPUT '?'-prompt suppression flag (MBASIC equiv of GBASIC's $0C94, +$23 shift). INPUT_PROMPT sets $FF (emit '? '); a trailing ',' clears it to 0 to suppress the question mark. INPUT_EMIT_PROMPT tests it. Was $0CB7. [RE]
INPUT_PROMPT_QMARK_FLAG:
        NOP                              ; $0CB7  00
; CHAIN string-compaction temp: CHAIN_COMPACT_STRINGS saves FRETOP (top-of-string) here ($5188) and restores it ($51D5) around the string-heap move that preserves COMMON variables. MBASIC equiv of GBASIC's $0C95 (+$23 shift). Was $0CB8. [RE]
CHAIN_FRETOP_SAVE:
        NOP                              ; $0CB8  00
        NOP                              ; $0CB9  00
; Default file record length / random-record limit cell. COLD_SET_WIDTH inits it to $0080 (128, the CP/M record size); FILE_NUM_TO_FCB range-checks the requested record number against it ($5BC4) and file setup re-stores it ($5FC7/$5FF8). MBASIC equiv of GBASIC's open-file-table-limit cell $0C97 (+$23 shift); the stale '$0CBA' in the FP comments is a transferred GBASIC note (real FP ARG temp is $0CDD). Was $0CBA. [RE]
FILE_RECLEN_DEFAULT:
        NOP                              ; $0CBA  00
        NOP                              ; $0CBB  00
; 'Running a stored program' / protected-program flag (MBASIC equiv of GBASIC's $0C99, +$23 shift). COLD_START and CLEAR_VARS clear it; ILLEGAL_DIRECT_CHECK ($5E2B) tests it (zero => Illegal direct error). LOAD/MERGE sets it to $FE (the SAVE,P protected marker) then triggers PROG_SCRAMBLE. Was $0CBC. [RE]
RUNNING_PROG_FLAG:
        NOP                              ; $0CBC  00
...
; [RE] CRUNCH continuation reached via LD BC,$307C/PUSH BC/RET: finishes tokenizing the reserved word that $6A41 matched, emitting its token byte via CRUNCH_EMIT and looping back to CRUNCH_1.
CRUNCH_RESWORD_TAIL:
        POP HL                           ; $1097  E1
...
CRUNCH_RESWORD_TAIL_8:
        ...
        POP AF                           ; $1132  F1
        XOR A                            ; $1133  AF
; [RE] Flag-skip into the JP-NZ operand. The CP nn / RET Z ladder above pushes CRUNCH_16+1 ($1135) as its return (LD BC,CRUNCH_16+1 / PUSH BC at $1104/$1107). A token MATCH returns into the operand of the cover, executing LD A,$01 then LD ($0B39),A (flag=1). NO match falls through POP AF / XOR A (A=0, Z set) -> the cover JP NZ,$013E is reached but NOT taken -> same LD ($0B39),A (flag=0). The C2 opcode byte of JP NZ is the one-byte cover both entrants step over; $0B39 records whether the prior token was a statement/clause introducer (CRUNCH mode flag, read at CRUNCH_19). VERIFIED: base_addr is real control flow (cover executed on fall-through), MBASIC twin byte-identical.
SUB_1128_1:
...
STMT_FOR_8:
        PUSH HL                          ; $1386  E5
; [RE] SMC console-status poll. CALL $0000 at $1387 is a placeholder; its operand ($1388-$1389 = STMT_FOR_8+1) is patched once at COLD_START. LD (STMT_FOR_8+1),HL at $5E77 stores HL = the BIOS CONST routine address (walked from the WBOOT vector at $0001 + offset 4) into the operand, the same address also stored to INKEY_SCAN_2+1 and RPC_CONST_POLL_1+1. Each NEWSTT statement then runs CALL CONST; POP HL; OR A; CALL NZ,INKEY_SCAN to poll for a pending key (Ctrl-C / break / pause). The on-disk $0000 is never executed. VERIFIED: operand really written ($5E77), patched CALL really executed per-statement (STMT_FOR_7 PUSH HL falls into it), MBASIC twin byte-identical.
STMT_FOR_9:
...
; CHRGOT embedded-token continuation: on the $10-class constant token, reload the saved text pointer (TERM_POS_WORK+$10, $0B3A) and re-enter CHRGOT. Part of the in-place CHRGET/CHRGOT char-fetch + constant-token expansion ($13E0 area). Was CHRGOT_RELOAD_PTR. [RE]
CHRGOT_RELOAD_PTR:
...
STMT_DEFSTR:
        LD E,$03                         ; $149F  1E 03
; [RE] Dual-entry type-code skip. DEFSTR (token $A9) enters at STMT_DEFSTR ($149F, LD E,$03); the $01 (LD BC,nn) opcodes act as 2-byte skips over each following LD E,nn. DEFINT (token $AA) is dispatched to STMT_DEFSTR_1+1 ($14A2), which decodes 1E 02 as LD E,$02 (type code 2). All four entries converge at STMT_DEFSTR_4 ($14AA). The DEFW is written +1 so it relocates.
STMT_DEFSTR_1:
...
; [RE] CHRGET then GETINT-positive: advances the text pointer, evaluates an expr to a signed 16-bit int (via $34CC); used where a leading char must be skipped first (e.g. WIDTH/coord parsers at $60DD/$60F5/$7F41/$7F7F)
GETINT_CHRGET_POS:
...
LINGET_NEXT:
        ...
        CP $0D                           ; $1504  FE 0D
; [RE] LINGET digit-accumulation loop: read ASCII digits, DE = DE*10 + digit (via the *10 sequence and $1998 overflow guard at $691F) until a non-digit; returns the parsed line number in DE.
LINGET_TOKLINE:
        EX DE,HL                         ; $1506  EB
...
STMT_RETURN_1:
        ...
        EX (SP),HL                       ; $15CC  E3
        LD A,$E1                         ; $15CD  3E E1
; [RE] DATA statement handler (token $84): no-op at run time (scanned/skipped). COMMON (token $B3) also dispatches to this same entry. Dual-entry skip idiom: the $01 (LD BC,nn) opcode doubles as a 2-byte skip. Entered here, the full LD BC,$0E5D executes; entered at STMT_DATA+2 ($15D1, called from ERROR_PRINT_SETUP) the embedded bytes start LD C,$00 ($0E $00) instead. Two paths share these bytes; the literal CALL $15D1 is written STMT_DATA+2 so it relocates.
; [RE] DATA/COMMON statement scanner with a dual-entry terminator skip. Entered at STMT_DATA ($15CF): LD BC,$0E5D loads C=':' so the scan stops at a colon (DATA item separator) -- token $84/$B3. Entered at STMT_DATA+2 ($15D1): the $01 opcode is sidestepped, 0E 00 decodes as LD C,$00 so B ends $00 and the scan runs to end-of-line only (REM-class, and the generic skip used by $0E17/$1570). The CALL/DEFW targets are written +2 so they relocate.
STMT_DATA:
...
INPUT_PROMPT_2:
        ...
        POP HL                           ; $1945  E1
        RET                              ; $1946  C9
; [RE] INPUT continuation: emits the '? ' prompt via OUTCHR, reads the console line ($702B), and on empty/abort branches to STMT_END; falls through to the value-parse loop $394D.
INPUT_EMIT_PROMPT:
        PUSH HL                          ; $1947  E5
...
FRMEVL_PAREN_4:
        ...
        POP HL                           ; $1CE5  E1
        RET                              ; $1CE6  C9
; [RE] Read char at (HL) and fold ASCII lowercase a-z ($61-$7A) to uppercase (AND $5F); leaves other chars unchanged. CRUNCH's case-insensitive keyword matcher uses this. $3CCD ($3CCD) is the same fold applied to the char already in A.
CHRGET_UPCASE:
        LD A,(HL)                        ; $1CE7  7E
...
FRMEVL_INT_OP_HANDLER_2:
        CP $3C                           ; $1E1B  FE 3C
; Continuation of FRMEVL_INT_OP_HANDLER (integer binary-operator dispatch): tests the operator token for XOR ($3C) and computes HL := HL XOR DE, else falls through to EQV/IMP. Was FRMEVL_INT_OP_XOR. [RE]
FRMEVL_INT_OP_XOR:
...
FRMEVL_INT_OP_XOR_2:
        ...
        CPL                              ; $1E39  2F
        RET                              ; $1E3A  C9
; [RE] 16-bit integer subtract (HL := HL - DE) then store the integer result into the FAC via FP_STORE ($51EE).
FP_INT_SUB_TO_FAC:
        LD A,L                           ; $1E3B  7D
        SUB E                            ; $1E3C  93
        LD L,A                           ; $1E3D  6F
...
; [RE] Print a $00-terminated message byte-by-byte through $6800 (console out); used by LIST to emit the post-line trailer text at $0A0E.
PRINT_ZSTRING:
        LD A,(HL)                        ; $211B  7E
        OR A                             ; $211C  B7
        RET Z                            ; $211D  C8
...
; [RE] LIST de-tokenizer: expand a crunched line back to ASCII into a buffer (BC=dest, D=remaining length). Copies literal bytes, and for reserved-word tokens (>= $0B) looks the keyword name up ($41E1) and copies its spelling. Used by LIST and by the sign-on/error formatting. $0C93 tracks inter-token spacing.
DETOKENIZE_LINE:
...
STMT_DELETE_2:
        ...
        EX (SP),HL                       ; $22AE  E3
; [RE] Copy a string from (DE) into the string pool growing at $0B6F, byte-by-byte via $691F until done; updates the $0B6F string-area pointer. Used by DELETE/edit to relocate text.
...
STMT_POKE:
        ...
        LD (DE),A                        ; $22DF  12
        RET                              ; $22E0  C9
; [RE] MS BASIC GETADR: evaluate FAC and convert to an unsigned 16-bit address in BC/$9180-bias form (for POKE/PEEK); rejects out-of-range via $4BA6. Pushes FN_LPOS as the integer-fetch continuation.
GETADR:
...
STMT_RENUM_8:
        CP $F6                           ; $238B  FE F6
; [RE] RENUM pass 2: walk every program line, find line-number references after GOTO/GOSUB/THEN tokens ($A4/$0E markers), translate each old line number to its new value ($34EB lookup) and rewrite the 3-byte encoded line-number token in place; reports 'Undefined line' for missing targets.
...
; [RE] 6502 remote-procedure-call dispatcher. HL = Apple monitor-ROM target address; stores it at A$VEC ($F3D0) then writes the trigger cell whose operand was self-modified at cold start to Z$CPU ($F3DE) (the 'LD ($0000),A' at $2605 is patched to 'LD ($F3DE),A' by the init at $5EC1). Storing to Z$CPU hands control to the 6502, which runs the Apple monitor routine and returns. This is the bridge graphics uses to reach the Apple ROM (TEXT/HOME/PREAD/etc.).
RPC_CALL:
        LD (A_VEC),HL                    ; $2602  22 D0 F3
; [RE] Self-modified store. Assembled as LD ($0000),A; cold-start init ($5EBE-$5EC3) reads Z$CPU ($F3DE) and patches this operand so it becomes LD (Z$CPU),A -- the write that actually triggers the 6502 to service the call queued at A$VEC.
; [RE] SMC operand patch (not a flag-skip). $2605 = LD ($0000),A; cold-start init at $5EC1 loads (Z$CPU)=$F3DE ($5EBE) and writes it into the operand at $2606, making the instruction LD ($F3DE),A. RPC_CALL runs the patched store to trigger the 6502 to service the routine queued at A$VEC ($F3D0). Both write AND later execute confirmed; MBASIC patches the same cell.
RPC_TRIGGER_STORE:
...
GFX_STMT_HLIN:
        ...
        POP HL                           ; $26A8  E1
        RET                              ; $26A9  C9
; [RE] VLIN statement body (mirror of GFX_STMT_HLIN): parse coords, store Y0/Y1->$F045/$F047 and X->$F02D, then fall to the low-res draw RPC ($F828) via STMT_CLEAR_1.
GFX_STMT_VLIN:
...
; [RE] FADD with operand at (HL): load the addend mantissa/exp into regs ($4EB8) then align-and-add against FAC.
FADD_FROM_MEM:
        CALL FP_LOAD_MEM                 ; $2819  CD 36 2B
        JR FADD_ALIGN                    ; $281C  18 06
FADD_FROM_MEM_1:
        CALL FP_LOAD_MEM                 ; $281E  CD 36 2B
; [RE] FADD entry when addend already supplied; loads FAC into HL-pair ($4E76 negate path) then aligns exponents.
FSUB:
...
; [RE] FP add/normalize core (FADDT path): align exponents, add/subtract mantissas, and renormalize the result in the FAC ($0CB1/$0CB4). $4C09 ($4C09) is the normalize/round tail that writes the final exponent to $0CB4.
FADD:
        LD L,B                           ; $2874  68
        LD H,E                           ; $2875  63
        XOR A                            ; $2876  AF
...
FMUL_2:
        ...
        PUSH DE                          ; $29C0  D5
; [RE] SMC operand cell: LD DE,nnnn at $29C1 is patched by FMUL setup ($299E LD (FMUL_3+1),HL) to the multiplicand mantissa low word; the shift-and-add loop then accumulates it via ADD HL,DE ($29C4). FMUL_3+1 is the immediate field, not a code entry.
FMUL_3:
        LD DE,$0000                      ; $29C1  11 00 00
        ADD HL,DE                        ; $29C4  19
        POP DE                           ; $29C5  D1
; [RE] SMC operand cell: the immediate of ADC A,nn at $29C6 is patched by FMUL prologue ($299A LD (FMUL_4+1),A) to the multiplicand high byte; folded into the running product high byte each loop pass. FMUL_4+1 is the immediate, not a code entry.
FMUL_4:
        ADC A,$00                        ; $29C6  CE 00
FMUL_5:
        RRA                              ; $29C8  1F
...
FMUL_8:
        ...
        LD C,A                           ; $29E6  4F
        RET                              ; $29E7  C9
; [RE] FDIV reciprocal-divide setup: pushes the divisor ($4E9A) and constant $5384, then falls into FDIV restoring-division.
FDIV_BY_TEN:
...
FDIV_BY_TEN_1:
        POP BC                           ; $29F1  C1
        POP DE                           ; $29F2  D1
; [RE] MBF floating-point divide: divisor self-modified into the subtract trio ($2A1B/$2A1F/$2A23), restoring long division of the FAC mantissa, result normalized via FADD_NORMALIZE.
FDIV:
...
FDIV_1:
        ...
        PUSH BC                          ; $2A19  C5
        LD A,L                           ; $2A1A  7D
; [RE] SMC operand cell: the SUB nn immediate at $2A1B is patched by FDIV setup ($2A0C LD (FDIV_2+1),A) to divisor mantissa byte 0; the restoring-division loop subtracts it from the remainder low byte. FDIV_2+1 is the immediate, not a code entry.
FDIV_2:
        SUB $00                          ; $2A1B  D6 00
        LD L,A                           ; $2A1D  6F
        LD A,H                           ; $2A1E  7C
; [RE] SMC operand cell: the SBC A,nn immediate at $2A1F is patched by FDIV setup ($2A07 LD (FDIV_3+1),A) to divisor mantissa byte 1; subtracted-with-borrow from the remainder middle byte. FDIV_3+1 is the immediate, not a code entry.
FDIV_3:
        SBC A,$00                        ; $2A1F  DE 00
        LD H,A                           ; $2A21  67
        LD A,B                           ; $2A22  78
; [RE] SMC operand cell: the SBC A,nn immediate at $2A23 is patched by FDIV setup ($2A02 LD (FDIV_4+1),A) to divisor mantissa byte 2; subtracted-with-borrow from the remainder high byte. FDIV_4+1 is the immediate, not a code entry.
FDIV_4:
        SBC A,$00                        ; $2A23  DE 00
        LD B,A                           ; $2A25  47
; [RE] SMC scratch cell: the LD A,nn immediate at $2A26 is the running quotient/borrow byte. Init 0 at $2A15; rewritten $2A2D/$2A64; read back LD A,(FDIV_5+1) at $2A3E/$2A60 to assemble the result. The byte is both the executed immediate and a self-modified scratch; FDIV_5+1 is not a code entry.
FDIV_5:
        LD A,$00                         ; $2A26  3E 00
        SBC A,$00                        ; $2A28  DE 00
        CCF                              ; $2A2A  3F
...
FN_INT:
        ...
        RET P                            ; $2AEA  F0
; [RE] Negate-FAC-and-continue: toggle the FAC sign ($4E76) then re-dispatch through the integer/type check ($3DC8) path.
FP_NEGATE_CHECKED:
...
FAC_PUSH:
        ...
        EX DE,HL                         ; $2B23  EB
        RET                              ; $2B24  C9
; [RE] Load operand at (HL) into regs ($4EB8) then store DE/BC into the FAC; combined load-then-store helper.
FP_STORE_REGS_LD:
...
; [RE] Load second operand from temp ($0CBA) and set up the type-driven move ($4EC9) before a compare/op.
FP_ARG_TO_TEMP2:
        LD HL,CHAIN_BREAK_FLAG_17        ; $2B6F  21 DD 0C
; [RE] Operand setup variant selecting the typed-move ($4EC9); type-check and pick temp source $0CAD/$0CB1.
FP_ARG_SETUP2:
...
FP_TO_INT_1:
        POP DE                           ; $2C54  D1
; [RE] Store HL into the FAC low cells ($0CB1) and set the value-type to $02 (single) in $0B14 ($4FDA). The MOVFR/integer-into-FAC primitive used throughout the evaluator.
FP_STORE_FAC_INT:
...
FIX_SCALE_4:
        ...
        CP $B8                           ; $2D41  FE B8
        RET NC                           ; $2D43  D0
; [RE] Denormalize toward the integer point for FIX: set exponent $B8, align the mantissa ($5306) and clear the guard cell $0CAC.
FIX_DENORM:
        PUSH AF                          ; $2D44  F5
...
; [RE] Float fallback for integer mul/div overflow: if result negative store integer (FP_STORE_FAC_INT $4FD7), else convert and re-enter via $500E and jump to $4E76 (FAC load).
...
INT_NEGATE_FAC:
        ...
        OR L                             ; $2E6A  B5
        RET NZ                           ; $2E6B  C0
; [RE] Promote integer FAC to single-precision: move to DE, set up via $502D, clear A, fall into FLOAT_FROM_INT.
INT_TO_SNG:
        EX DE,HL                         ; $2E6C  EB
...
; 16-bit integer DIVIDE: multiply-by-reciprocal then RRA-shift the quotient (D:E) into H:L with rounding, store via $4FDA and INT_ABS_STORE. MS BASIC-80 integer divide.
INT_DIV_ROUND:
        PUSH DE                          ; $2E76  D5
...
DP_SHIFT_LEFT_8:
        LD C,$08                         ; $2FB2  0E 08
; Loop body of DP_SHIFT_LEFT_8: chained RLA across the 8 mantissa bytes from (HL), one bit left, used between partial products in the double-precision multiply (also CALLed at $30DC). Was DP_SHIFT_LEFT_8_LOOP. [RE]
...
; Double-precision DIVIDE: handle exponent/sign, restoring-division by 15 iterations (DP_PUSH_OPERAND / DADD / DP_POP_OPERAND), building the quotient mantissa; promotes to single ($4E76) when exponent small. MS BASIC-80 double divide.
FIN_DSCALE_DIV10:
...
DDIV_2:
        ...
        XOR A                            ; $30C9  AF
; [RE] DDIV digit-loop overlap (VERIFIED). DA 12 04 = JP C,$0412 only on the carry-CLEAR fall-through ($30C9 XOR A guarantees carry clear, so the JP is a never-taken 2-byte skip of its own operand). The carry-SET branch (JR C,DDIV_2+1 at $30C2) enters at +1 and runs 12 04 = LD (DE),A / INC B: store the reduced remainder, count one decimal digit. Both paths land at $30CD. MBASIC DDIV_3 byte-identical.
DDIV_3:
...
; [RE] FIN flag-skip (VERIFIED). F6 AF = OR $AF on FIN's top entry can never give zero, so Z stays clear and the later CALL Z,FP_STORE_FAC_INT ($3137) is skipped (parse from FAC=0). The five +1 entrants run the operand AF = XOR A, setting Z so FP_STORE_FAC_INT pre-seeds the FAC integer first. Z is carried across PUSH AF/POP AF ($312A/$3130). The OR operand $AF is exactly the XOR A opcode. MBASIC FIN_1 byte-identical.
FIN_1:
        OR $AF                           ; $3124  F6 AF
...
FIN_TYPE_FIXUP:
        ...
        POP HL                           ; $3200  E1
        RET                              ; $3201  C9
; [RE] FIN accumulate digit *10: RET if count zero, else multiply the running value by ten using the single-precision ($4E30) or double-precision (DP_MUL10 $5488) path per type parity; returns A decremented.
FIN_MUL10:
        RET Z                            ; $3202  C8
...
DEC_A_RET:
        DEC A                            ; $3210  3D
        RET                              ; $3211  C9
; [RE] FIN fractional scale-down: divide the running value by ten via the single-precision ($4D6A) or double-precision (DDIV $5388) path per type parity; returns A incremented to track the decimal exponent.
FIN_DIV10:
        PUSH DE                          ; $3212  D5
        PUSH HL                          ; $3213  E5
        PUSH AF                          ; $3214  F5
...
; [RE] FIN finalize: store the completed FAC ($4E9A / $4E56), unwind the saved registers and dispatch through $4BA6 to return the converted number to the expression evaluator.
FIN_DONE:
...
FIN_EXP_DIGIT:
        ...
        SUB $30                          ; $329E  D6 30
        LD E,A                           ; $32A0  5F
; [RE] FIN exponent-overflow flag-skip (VERIFIED). On the accumulate path (E<10) FA 1E 7F = JP M,$5B9C never triggers (E*10+digit stays positive, <100) and just skips its operand to reach JP FIN_8. The E>=10 branch (JR NC,FIN_DONE_3+1 at $3297) enters at +1 to run 1E 7F = LD E,$7F, saturating the exponent so the literal overflows. Both converge at $32A4. MBASIC FIN_EXP_DIGIT_1 byte-identical.
FIN_EXP_DIGIT_1:
...
FOUT_EXPONENT_12:
        ...
        DEC HL                           ; $35A1  2B
; [RE] FOUT trailing-zero strip-loop overlap (VERIFIED). 01 2B 77 = LD BC,$53A9 runs once on first entry; the JR Z self-loop (28 FB at $35A6) re-enters at +1, skipping the 01 opcode (BC kept) and re-executing 2B 77 = DEC HL / LD (HL),A as the loop body. The LD BC operand bytes double as the strip-and-overwrite step. MBASIC FOUT_EXPONENT_13 byte-identical.
FOUT_EXPONENT_13:
...
FOUT_EXPONENT_27:
        ...
        LD E,$10                         ; $3657  1E 10
; [RE] FOUT field-select flag-skip (VERIFIED, reached_cover refined). The NZ entry (JP C,FOUT_EXPONENT_25 at $35B5 reaching $3654 with Z clear, then $3657 LD E,$10) runs 01 1E 06 = LD BC,$061E; the Z entry (JP Z,FOUT_EXPONENT_26+1 at $3654) enters at +1 and runs 1E 06 = LD E,$06, skipping the LD BC opcode. So E becomes $10 (16) vs $06 (6). Both converge at $365C CALL FP_SIGN. Note: the local $3653 XOR A always forces the +1 path; the cover is exercised only via the external $35B5 NZ entry. MBASIC FOUT_EXPONENT_29 byte-identical.
FOUT_EXPONENT_28:
...
POW10_INT_TABLE_1:
        ...
        LD B,A                           ; $38BD  47
; [RE] HEX$/OCT$ base-selector flag-skip (VERIFIED). Octal entry ($38BC) does XOR A / LD B,A (B=0) then C2 06 01 = JP NZ,$0106, never taken (Z set), skipping its operand so B stays 0. Hex callers (JP/CALL HEX_OCT_OUT_1+1 at $221D/$4832) enter at +1 and run 06 01 = LD B,$01 (B=1). DEC B/INC B at $38CC then picks C=$06 octal groups vs C=$04 hex nibbles. Converge at $38C1 PUSH BC. MBASIC twin (POW10_INT_TABLE region) byte-identical.
POW10_INT_TABLE_2:
...
; [RE] VERIFIED flag-skip. POLY_EVAL_1 ($39EB) opcode-eating dual entry. First pass (fall-through from FP_STORE_REGS_LD CALL at $39E8) executes LD B,$F1 (06 F1); the $06 opcode swallows the $F1, so the first iteration pops only BC,DE. The Horner loop re-enters via JR POLY_EVAL_1+1 ($39EC), running $F1 as POP AF to discard the per-iteration PUSH AF ($39F3) before popping BC,DE. B=$F1 is dead (overwritten by POP BC at $39ED). Cover is genuinely executed on fall-through; MBASIC twin byte-identical.
POLY_EVAL_1:
        LD B,$F1                         ; $39EB  06 F1
        POP BC                           ; $39ED  C1
        POP DE                           ; $39EE  D1
...
PTRGET:
        ...
        PUSH BC                          ; $3BB1  C5
; [RE] VERIFIED flag-skip (count corrected to 22 CALL +1 sites, not 23). PTRGET_1 ($3BB2) opcode-eating flag select. Fall-through from the PTRGET head -- reached via the DIM statement dispatch DEFW PTRGET at $0112 -- runs OR $AF (F6 swallows the AF), leaving A nonzero so $0B36 gets the 'array/create' context; the 22 CALL PTRGET_1+1 ($3BB3) callers run that AF as XOR A, zeroing A and $0B36. Both reach LD ($0B36),A at $3BB4. Cover genuinely executed (DIM path); MBASIC twin matches (22 sites).
PTRGET_1:
        OR $AF                           ; $3BB2  F6 AF
...
; [RE] PTRGET search/allocate core: walk the simple-variable table ($0B6F..$0B71) for the packed name in C/$0B14/$0871; on a hit return its address, on a miss create a new entry (allocate via STR/var-space grow, $6811/$622E). Detects '(' to branch to the array path (subscript eval + array search/alloc, $612C loop) honouring DIM and OPTION BASE.
PTRGET_SEARCH:
        XOR A                            ; $3C47  AF
...
SUB_3D4E_5:
        ...
        LD E,$00                         ; $3DA7  1E 00
        PUSH DE                          ; $3DA9  D5
; [RE] VERIFIED flag-skip. PTRGET_SEARCH_23 ($3DAA) opcode-eating entry. Fall-through (from PTRGET_SEARCH_22 PUSH DE at $3DA9) executes LD DE,$F5E5; the $11 opcode swallows E5 F5 so the first pass skips PUSH HL/PUSH AF and loads a dead DE. The two +1 entrants ($3DAB, JP Z at $3C36 / CALL at $5007) run E5 F5 as PUSH HL / PUSH AF to save caller state. Both reach LD HL,($0B94) at $3DAD. Cover genuinely executed; MBASIC twin ($1D69, still machine-labeled) byte-identical.
SUB_3D4E_6:
        LD DE,$F5E5                      ; $3DAA  11 E5 F5
        LD HL,(VARTAB_1)                 ; $3DAD  2A 94 0B
; [RE] VERIFIED flag-skip. PTRGET_SEARCH_24 ($3DB0) opcode-eating loop advance. Fall-through (from $3DAD) runs LD A,$19 (3E swallows 19); the first array-table pass skips ADD HL,DE because HL is already at the table head. The loop re-enters via JR NZ,PTRGET_SEARCH_24+1 ($3DB1) at $3DD7, running 19 as ADD HL,DE to step to the next entry. A=$19 is dead (clobbered at $3DBE). Both reach EX DE,HL / CMP_HL_DE at $3DB2. Cover genuinely executed; MBASIC twin ($1D69) byte-identical (20 D8).
SUB_3D4E_7:
        LD A,$19                         ; $3DB0  3E 19
        EX DE,HL                         ; $3DB2  EB
...
SUB_3D4E_20:
        ...
        LD A,(HL)                        ; $3E6B  7E
        INC HL                           ; $3E6C  23
; [RE] VERIFIED flag-skip. PTRGET_SEARCH_36 ($3E6D) opcode-eating dimension loop. Fall-through (from PTRGET_SEARCH_35) runs LD D,$E1 (16 swallows E1); the first array dimension skips POP HL because HL is already valid. The loop re-enters via JR NZ,PTRGET_SEARCH_36+1 ($3E6E) at $3E83, running E1 as POP HL to restore the per-dimension HL save (EX (SP),HL/PUSH AF at $3E73/$3E74). D=$E1 is dead (clobbered at $3E71). Both reach LD E,(HL) at $3E6F. Cover genuinely executed; MBASIC twin ($1D69) byte-identical (20 E9).
SUB_3D4E_21:
        LD D,$E1                         ; $3E6D  16 E1
        LD E,(HL)                        ; $3E6F  5E
        INC HL                           ; $3E70  23
...
; [RE] Line-editor helper: echo D characters of the line buffer at (HL) to the console (OUTCHR via $6800), advancing HL and the column count B; ESC ($1B) enters the copy/scan sub-mode (EDIT_COPY_MODE). Part of the EDIT/line-input screen editor.
EDIT_ECHO_SPAN:
        LD A,(HL)                        ; $3FA4  7E
        OR A                             ; $3FA5  B7
        RET Z                            ; $3FA6  C8
...
; [RE] PRINT USING statement engine (reached from the PRINT dispatcher on token $E8 'USING'): evaluate the format string, then scan its field characters - '#' digit positions, '.' decimal point, ',' grouping, '+'/'-' sign, '$$' float-dollar, '**' asterisk-fill, '^^^^' exponential, '\ \' string field, '!' first-char, '&' variable string - formatting each argument (FOUT/$5723 for numbers, STROUT for strings) and looping over the value list.
PRINT_USING:
...
PRINT_USING_10:
        ...
        DEC B                            ; $4167  05
        INC E                            ; $4168  1C
; [RE] VERIFIED flag-skip. PRINT_LIST_ENTRY_13 ($4169) opcode-eating field seed. Fall-through (numeric-digit path via $4167 DEC B / $4168 INC E) runs CP $AF (FE swallows AF), a flags-only compare leaving the digit char in A before ADD A,$10. The floating-currency '$$' entry JR Z,PRINT_LIST_ENTRY_13+1 ($416A) at $4154 runs AF as XOR A, so ADD A,$10 starts from $10. Both reach ADD A,$10 / INC HL at $416B. Cover genuinely executed; MBASIC twin (PRINT_USING_11) byte-identical (28 14).
PRINT_USING_11:
        CP $AF                           ; $4169  FE AF
        ADD A,$10                        ; $416B  C6 10
        INC HL                           ; $416D  23
...
PRINT_USING_16:
        ...
        INC D                            ; $41BA  14
        INC HL                           ; $41BB  23
; [RE] VERIFIED flag-skip (with nuance). PRINT_LIST_ENTRY_19+1 ($41BD) is a RET landing pad, not a JP target. The '^^^^' field parser pushes $41BD ($41A1/$41A4) and saves HL into DE ($41A5); the RET NZ/RET C chain returns to $41BD to run EX DE,HL / POP DE and unwind. The cover at $41BC (CA EB D1) is a genuine, executed JP Z,$D1EB whose Z comes from INC D ($41BA) and whose $D1EB target is unreachable BIOS-region junk, so it is effectively never taken -- the CA opcode hides the EB D1 (EX DE,HL/POP DE) on the fall-through. Both reach PRINT_LIST_ENTRY_20 ($41BF). MBASIC twin (PRINT_USING_17) byte-identical.
PRINT_USING_17:
...
PRINT_USING_30:
        LD C,$01                         ; $424D  0E 01
; [RE] VERIFIED flag-skip. PRINT_LIST_ENTRY_34 ($424F) opcode-eating stack rebalance. Fall-through (numeric/sign field, from PRINT_LIST_ENTRY_33 $424D) runs LD A,$F1 (3E swallows F1) so no POP; A=$F1 is dead. The '\' string-field entry JP Z,PRINT_LIST_ENTRY_34+1 ($4250) at $410A runs F1 as POP AF to drop the PUSH the scanner left. (PRINT_LIST_ENTRY_32 at $4249 JRs straight to _35 at $4251, bypassing the cover.) Both reach DEC B / CALL PRINT_USING_PUT_SIGN at $4251. Cover genuinely executed on fall-through; MBASIC twin (PRINT_USING_31) byte-identical.
PRINT_USING_31:
        LD A,$F1                         ; $424F  3E F1
PRINT_USING_32:
        DEC B                            ; $4251  05
...
PRINT_USING_PUT_SIGN:
        ...
        POP AF                           ; $428F  F1
        RET                              ; $4290  C9
; [RE] OUTCHR: console character output with column tracking ($0837 cursor column), TAB ($09) expansion to 8-col stops, backspace ($08) and CR handling, then emits the byte through the BIOS console-out vector ($666C, CALL into the runtime-patched $0000 cell).
OUTCHR:
        PUSH AF                          ; $4291  F5
        PUSH HL                          ; $4292  E5
...
OUTDO_DEVICE:
        ...
        PUSH HL                          ; $42ED  E5
        LD C,A                           ; $42EE  4F
; [RE] VERIFIED smc. OUTDO_DEVICE_1+1 ($42F0) is the SMC operand of CALL $0000 ($42EF), reached by fall-through from OUTDO_DEVICE ($42EA). COLD_START LD (OUTDO_DEVICE_1+1),HL at $5E95 patches it to the live CP/M BIOS CONOUT entry (walked from $0001 through the BIOS jump table). The CD 00 00 is a placeholder; +1 is the patch slot, written never jumped-to, and the patched CALL executes on every console output. MBASIC twin patches at $3B13.
OUTDO_DEVICE_1:
...
OUTDO_RESET_COL:
        ...
        OR A                             ; $42FE  B7
        RET Z                            ; $42FF  C8
; [RE] High-level char-out with line-width/auto-CR logic: enforces the terminal width, expands TAB, handles backspace against the column counter ($0B11), and issues CRLF when the print column reaches the width. Wraps the OUTCHR primitive ($6704).
OUTDO_WIDTH:
        LD A,$0D                         ; $4300  3E 0D
...
OUTDO_DEVICE2:
        ...
        PUSH HL                          ; $4385  E5
        LD C,A                           ; $4386  4F
; [RE] VERIFIED smc. OUTDO_DEVICE2_1+1 ($4388) is the SMC operand of CALL $0000 ($4387), reached by fall-through from OUTDO_DEVICE2 ($4382). COLD_START LD (OUTDO_DEVICE2_1+1),HL at $5E8B patches it to the live CP/M BIOS list/raw-output entry (from the BIOS jump-table walk). +1 is the patch slot, not a code entry; the patched CALL executes at runtime. MBASIC twin patches at $3B09.
OUTDO_DEVICE2_1:
        ...
        POP AF                           ; $438D  F1
        RET                              ; $438E  C9
; [RE] Force end-of-line bookkeeping: calls $6792 to clear column state, then falls into LINE_COUNT_INC to bump the printed-line counter / page check ($0B12 vs $083C).
LIST_NEWLINE_COUNT:
...
LINE_COUNT_INC_1:
        XOR A                            ; $43A0  AF
        RET                              ; $43A1  C9
; [RE] Auto-page 'more' handler on the LIST/output path: skips when output is redirected ($0840 nonzero), tests pending key ($7C0B), runs the input poll ($781A) and Ctrl-C/break check ($0CA0), and on a full page pushes STMT_FOR_7 and prints the pause prompt string at $0CF2 via STROUT before resuming.
INCHR:
        PUSH HL                          ; $43A2  E5
...
CONIN:
        ...
        PUSH DE                          ; $43DB  D5
        PUSH HL                          ; $43DC  E5
; [RE] VERIFIED smc. CONIN_1+1 ($43DE) is the SMC operand of CALL $0000 ($43DD), reached by fall-through from CONIN ($43DA). COLD_START LD (CONIN_1+1),HL at $5E81 patches it to the live CP/M BIOS CONIN entry (from the BIOS jump-table walk). +1 is the patch slot; the patched CALL executes on every console read. MBASIC twin patches at $3AFF.
CONIN_1:
...
RPC_CONST_POLL:
        ...
        PUSH DE                          ; $442D  D5
        PUSH HL                          ; $442E  E5
; [RE] VERIFIED smc. RPC_CONST_POLL_1+1 ($4430) is the SMC operand of CALL $0000 ($442F), reached by fall-through from RPC_CONST_POLL ($442C). COLD_START LD (RPC_CONST_POLL_1+1),HL at $5E74 patches it to the live CP/M BIOS CONST entry (BIOS base+4, the same HL written to INKEY_SCAN_2+1 at $5E71). +1 is the patch slot; the patched CALL executes when polling console status. MBASIC twin patches at $3AF2.
RPC_CONST_POLL_1:
        ...
        OR A                             ; $4435  B7
        RET Z                            ; $4436  C8
; [RE] Keyboard scan / pending-char handler (INKEY$ / Ctrl-C-Ctrl-S poll): reads a console char ($675C), processes the Ctrl-S ($13) pause and Ctrl-C ($03) break ($0834 pending-key cell), and returns it. SUB_67B9_1 ($67CC) is the INKEY$ function evaluator.
INKEY_SCAN:
...
; [RE] VERIFIED smc. INKEY_SCAN_2+1 ($4454) is the SMC operand of CALL $0000 ($4453), reached by fall-through from INKEY_SCAN_1 ($444A) on the no-pending-key path. COLD_START LD (INKEY_SCAN_2+1),HL at $5E71 patches it to the live CP/M BIOS CONST entry for INKEY$ polling (BIOS base+4, same HL also written to RPC_CONST_POLL_1+1 at $5E74). +1 is the patch slot; the patched CALL executes when polling for a key. MBASIC twin uses the same COLD_START BIOS-table walk.
INKEY_SCAN_3:
...
; [RE] String copy loop without the prior heap check: move bytes from (HL) to (BC) decrementing both until HL==DE ($691F compare). Tail of STR_COPY_DOWN.
STR_COPY_DOWN_NOCHK:
        PUSH BC                          ; $4492  C5
        EX (SP),HL                       ; $4493  E3
        POP BC                           ; $4494  C1
...
; [RE] String free/space guard: if the requested string allocation would collide with the variable space, trigger garbage collection ($6C82) and retry; if still no room raise 'Out of string space' (E=$07/$0E) via RAISE_ERROR.
...
; [RE] Dual entry. Fall-through (END/stop tail from STMT_END_1) runs LD HL,$FFF6 whose value is dead (RESUME_AT_DIRECT at $45E7 reloads HL from SAVTXT); its only job is to eat the F6 FF bytes so A is preserved. The three JP C,STMT_END_2+1 sites (after INLIN returns carry = input aborted) enter +1 so F6 FF runs as OR $FF, flagging the break in A=$FF. Both merge at $45E6 POP BC.
STMT_END_2:
...
; [RE] Finalize a string-returning function result: scan the formed string ($6BEA), build its descriptor and stage it as the string temporary returned through $6E0B.
STR_FN_FINALIZE:
...
STR_FN_FINALIZE_1:
        ...
        PUSH BC                          ; $4843  C5
; [RE] Build/allocate a string body from a descriptor: for the byte at (HL) call GETSPA to reserve space, copy via $6DB0, returning the new descriptor; used by string concatenation/formatting.
STR_BUILD_FROM_DESC:
        LD A,(HL)                        ; $4844  7E
        INC HL                           ; $4845  23
        PUSH HL                          ; $4846  E5
...
; Store-string-descriptor core with HL pre-loaded to the target descriptor cell: writes length A and body pointer DE at (HL), returns HL. STORE_STR_DESC ($485D) enters here with HL = temp-descriptor $0B68; also CALLed directly ($1006). Was STORE_STR_DESC_AT_HL. [RE]
...
; [RE] FRESTR/movestring helper: pull a string's descriptor (via FRMEVL_EVAL_OPERAND), free its data with $6DC0, then copy the bytes into freshly-allocated string space ($6DA8 block copies); returns through FRMEVL fixup.
STR_CONCAT:
        PUSH BC                          ; $49EE  C5
        PUSH HL                          ; $49EF  E5
...
; [RE] copy one string-descriptor's data: pops the descriptor (len in A, addr in BC) off the caller's stack and block-copies len bytes from (BC) to (DE) via $6DB0.
STR_COPY_DESCR_DATA:
        POP HL                           ; $4A26  E1
        EX (SP),HL                       ; $4A27  E3
        LD A,(HL)                        ; $4A28  7E
...
; MS BASIC-80 FRESTR: free the string whose descriptor pointer is in FAC ($0CB1); loads the descriptor then frees its data via FRESTR1 ($6DC0).
FRESTR:
...
; [RE] string-function epilogue: store result char (E) into the string work buffer at ($0B46) and return through $6C1A (FRMEVL string-temp fixup).
STR_FN_RETURN_CHAR:
...
FN_PEEK:
        ...
        LD A,$20                         ; $4ABD  3E 20
; [RE] STRING$/SPACE$ build helper: allocate B bytes of string space, fill with the pad char in A, returning the new descriptor; entry from the function epilogue at $6E0B.
STR_FILL_ALLOC:
        PUSH AF                          ; $4ABF  F5
        LD A,E                           ; $4AC0  7B
...
INLIN_PUT_AND_RESET:
        LD (HL),B                        ; $4CA0  70
; INLIN editor state reset (entered just below INLIN_PUT_AND_RESET): clear the pending-control self-mod byte ($0857) and the INLIN_REDISPLAY_FLAG ($0CB6) before reading a fresh input line. Was INLIN_RESET_EDIT_STATE. [RE]
...
; [RE] Ctrl-R / retype-line: terminate the buffer, CRLF, redisplay the line accumulated so far ($4100) from $0A0E, then resume editing.
INLIN_CTRL_R:
        CP $12                           ; $4D44  FE 12
...
; [RE] snapshot the current print column ($0B34) into the Tab-expansion base cell (self-modified operand at $4E1B) so Tab stops align to where the prompt left the cursor.
INLIN_SAVE_COLUMN:
...
; [RE] CHAIN preserve pass: relocate kept simple/array variables, copying their bytes up out of the way (via $691F compare and block move) and adjusting the variable/array area pointers ($0B71/$0B73) so they survive the new program load.
...
CHAIN_COPY_VAR_BLOCK_1:
        ...
        RET                              ; $50C0  C9
; [RE] CHAIN preserve pass over arrays: walk the array table, temporarily clearing each entry's preserve bit, summing sizes ($6225) and relocating the kept ones.
CHAIN_SCAN_ARRAYS:
...
; [RE] CHAIN preserve pass over string variables/arrays: walk simple-variable and array storage ($0B6F/$0B71), freeing/relocating string descriptors (type 3) of kept variables via $74C5.
CHAIN_SCAN_STRINGS:
...
CHAIN_SCAN_STRINGS_5:
        ...
        PUSH BC                          ; $5142  C5
; [RE] move one kept string's data during CHAIN: if the string lives in program/heap space that the reload will clobber, copy it up into safe string space ($6BC6/$4ECD) and rewrite its descriptor pointer.
...
FN_LOF:
        LD A,$02                         ; $52DF  3E 02
; [RE] LOF op-selector flag-skip. 01 here is the LD BC,nn skip-prefix, not a real load: falling through FN_LOF ($52DF LD A,$02) the 01 swallows 3E 04, so A stays $02. FUNC_DISPATCH_TBL $0214 (FN_LOF_1+1, $52E2) instead runs 3E 04 = LD A,$04. Converge at $52E7 PUSH AF -> CALL FRMEVL_APPLY_OP.
FN_LOF_1:
        LD BC,$043E                      ; $52E1  01 3E 04
; [RE] LOF op-selector flag-skip, arm 3. 01 = LD BC,nn skip-prefix swallowing 3E 08; entrants from FN_LOF/FN_LOF_1+1 keep their A. FUNC_DISPATCH_TBL $0216 (FN_LOF_2+1, $52E5) runs 3E 08 = LD A,$08. Converge at $52E7 PUSH AF.
FN_LOF_2:
...
FN_CVI:
        LD A,$01                         ; $52F8  3E 01
; [RE] CVI/CVS/CVD width-selector flag-skip. 01 = LD BC,nn skip-prefix swallowing 3E 03; FN_CVI ($52F8 LD A,$01) keeps A=$01. FUNC_DISPATCH_TBL $0206 (FN_CVI_1+1, $52FB) runs 3E 03 = LD A,$03. Converge at $5300 PUSH AF; A is the min-string-length checked at $5305 CP (HL).
FN_CVI_1:
        LD BC,$033E                      ; $52FA  01 3E 03
; [RE] CVD width-selector flag-skip, arm 3. 01 = LD BC,nn skip-prefix swallowing 3E 07; entrants from FN_CVI/FN_CVI_1+1 keep their A. FUNC_DISPATCH_TBL $0208 (FN_CVI_2+1, $52FE) runs 3E 07 = LD A,$07. Converge at $5300 PUSH AF.
FN_CVI_2:
...
; [RE] LOAD/RUN entry flag-skip. JP NZ,OPEN_NAMED_FILE_1 ($153D/$5E49) reaches $53FD OR $AF -> A nonzero (run after load). STMT_DISPATCH_TBL $0180 (OPEN_NAMED_FILE_1+1, $53FE) runs the swallowed AF = XOR A -> A=0 (plain LOAD). Converge at $53FF PUSH AF.
OPEN_NAMED_FILE_1:
        OR $AF                           ; $53FD  F6 AF
        PUSH AF                          ; $53FF  F5
...
; [RE] OPEN open-mode flag-skip. Fall-through reaches $541F OR $F1 -> A nonzero. JR Z,OPEN_NAMED_FILE_3+1 ($540D) enters $5420 = the swallowed F1 = POP AF, restoring the saved flag. Converge at $5421 LD ($086D),A.
OPEN_NAMED_FILE_4:
        OR $F1                           ; $541F  F6 F1
...
STMT_SAVE_1:
        ...
        LD A,$FF                         ; $5501  3E FF
; [RE] SAVE core: write the tokenized program image (line links + text) byte-by-byte to the open output file via PUTC_FILE ($7B22), terminating at end-of-program.
SAVE_WRITE_PROGRAM:
...
; [RE] RSET statement handler (token $C3): right-justify a string into a FIELD buffer variable. LSET (token $C2) enters at $55BC with the justify flag cleared.
; [RE] LSET/RSET justify flag-skip via carry. RSET (dispatch $018C) enters $55BB OR $37 -> CF cleared. LSET (dispatch $018A, STMT_RSET+1 $55BC) runs the swallowed 37 = SCF -> CF set. Both PUSH AF at $55BD; downstream reads carry for pad direction.
STMT_RSET:
        OR $37                           ; $55BB  F6 37
        PUSH AF                          ; $55BD  F5
...
; [RE] SMC: SYSTEM-exit JP target. Image holds JP $0000 at $5A6B; COLD_START ($5E5F LD HL,($0001) -> $5E62 LD (STMT_SYSTEM_WBOOT+1),HL) writes the CP/M BIOS WBOOT vector into the C3 operand. Running SYSTEM (reached from $0E3B or fall-through) then JP <WBOOT> to leave BASIC.
STMT_SYSTEM_1:
...
STMT_RESET:
        ...
        POP HL                           ; $5A85  E1
        RET                              ; $5A86  C9
; [RE] KILL statement: parse filename into the default FCB ($0080/$08AA) then BDOS delete-file (C=$13); '?' from BDOS -> error $0D4A. Reached via dispatch DEFW $7E09 at $7A58.
STMT_KILL:
        ...
        POP HL                           ; $5AAF  E1
        RET                              ; $5AB0  C9
; [RE] FILES statement: directory listing. Optional filespec parsed into FCB ($08AA), '*' expanded to '?' ($7EBB), BDOS set-DMA ($1A) + search-first ($11)/search-next ($12); prints each 11-char name with '.' between name and extension via OUTCHR, columns from print-col ($083B).
STMT_FILES:
...
BDOS_FILE_CALL_2:
        POP AF                           ; $5B6E  F1
        RET                              ; $5B6F  C9
; [RE] Sequential file read into the data buffer: set DMA via FCB cursor, loop BDOS read-sequential ($14) of 128-byte records ($7F25 sets FCB ptr), copying each record into the user buffer until DE count exhausted.
FILE_READ_RECORDS:
        EX DE,HL                         ; $5B70  EB
...
; [RE] Compute the FCB working pointer (current file-table entry $0B48 - $2A*1 reverse offset) and verify stack room ($691F/$781A); used before each BDOS sequential transfer.
...
FILE_NUM_TO_FCB_2_2:
        ...
        POP DE                           ; $5BDD  D1
        RET                              ; $5BDE  C9
; [RE] PUT statement handler (token $BB): write a random-file record. GET (token $BA) enters one byte later at $5BE0.
; [RE] GET/PUT direction flag-skip. PUT (dispatch $017C) enters $5BDF OR $AF -> A nonzero (write). GET (dispatch $017A, STMT_PUT+1 $5BE0) runs the swallowed AF = XOR A -> A=0 (read). Stored at $5BE1 ($5E3A), read later to select read vs write.
STMT_PUT:
        OR $AF                           ; $5BDF  F6 AF
...
; [RE] Random-file PUT inner helper: walk the FIELD descriptor chain (FCB+$AB pointer pair), advance the write cursor (ILLEGAL_DIRECT_CHECK_1), and on buffer-full dispatch the record write (PUTC_FILE_3 at $57DB). Entered at $5CF5 (skip the OR $AF flag-set) for the no-flag variant.
; [RE] FIELD record-write flag-skip. CALL FIELD_WRITE_RECORD ($5CBE) hits $5CF4 OR $AF -> A nonzero. CALL FIELD_WRITE_RECORD+1 ($5CA7/$5CDA) runs the swallowed AF = XOR A -> A=0. Stored at $5CF6 ($086F), tested at $5D13 to gate the record-write dispatch.
FIELD_WRITE_RECORD:
        OR $AF                           ; $5CF4  F6 AF
...
; Common register-restore epilogue of FIELD_WRITE_RECORD (POP HL/DE/BC; RET). The DEFW at $019F referencing it is a statement-dispatch-table misalignment artifact, not a real handler. Was FIELD_WRITE_RECORD_RET. [RE]
...
SUB_5EBE_1:
        ...
        LD A,$28                         ; $5F02  3E 28
; [RE] Select terminal line width during cold start: reads the configured console type ($F3BB) and sets the line-width work cell (GFX_READ_COORD_PAIR_12, $2815) to 40 ($28) or the wide default, then initializes the file-control / disk-parameter pointers (WIDTH_SET_CONSOLE).
; [RE] 40/80 line-width flag-skip. Default console falls through $5F02 LD A,$28 (40), and 01 at $5F04 swallows 3E 50 so A stays $28. JR Z,COLD_SET_WIDTH+1 ($5EFD/$5F00, console type 3/4) enters $5F05 = LD A,$50 (80). Converge at $5F07 store of the width cell.
COLD_SET_WIDTH:
