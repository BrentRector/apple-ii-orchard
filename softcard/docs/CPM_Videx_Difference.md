...
1. Running `python -m disasm6502` on a 3 KB Apple II memory image
   ...
   $1100, $1200, $1300).
2. Both `softcard/disks/CPMV233.DSK` (CP/M 2.23, DOS 3.3 sector order) and
   `softcard/disks/CPM220Disk1.po` (CP/M 2.20, ProDOS sector order) yield the
...
3. The build scripts and intermediate artifacts live in
   `softcard/cpm-investigation/`.

Files of interest:

| File | Purpose |
|------|---------|
| `softcard/cpm-investigation/extract_loader.py` | Builds the 3 KB Apple II memory image |
| `softcard/cpm-investigation/loader_223.bin` | 2.23 boot loader image ($0800-$13FF) |
| `softcard/cpm-investigation/loader_220.bin` | 2.20 boot loader image ($0800-$13FF) |
| `softcard/cpm-investigation/disasm_223_linear.txt` | Full 6502 linear disassembly |
| `softcard/cpm-investigation/disasm_220_linear.txt` | Full 6502 linear disassembly |
