...
3. **BIOS base off-by-$B8 (2.23) / off-by-$CC (2.20).** The BIOS jump
   ...
   - `softcard/docs/CPM223_BIOS.asm` and `CPM220_BIOS.asm` have since been
...
- `softcard/cpm-investigation/emu_softcard_realmap.py` — one-shot probe: 6502
  ...
  reaching BDOS.
- `softcard/cpm-investigation/emu_softcard_v2.py` — THE bidirectional emulator:
...
hook). Contract captured from softcard/docs/CPM223_RWTS.asm:
...
- `softcard/cpm-investigation/videx_rom24.s` — disasm6502 output of Videx ROM
