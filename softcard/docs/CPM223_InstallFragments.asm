...
; [AI] Table of 16-bit little-endian Z-80 BIOS handler addresses ($FB14, $FB33, $FCB5, $FE66,
;       $FE60, $FE4C), each duplicated; the Z-80 cold-boot generator indexes it by device code to
;       find the right CONIN/CONOUT/status handler for each device class.
DEV_HANDLER_TABLE:
...
; [AI] Mixed slot/dispatch metadata read by both the 6502-side dispatch and the Z-80 cold-boot
;       generator; encodes device-code-keyed table indices and slot bits used to wire console I/O to
;       the detected card.
SLOT_INFO_BLOCK:
...
; [AI] Eight bytes ($03B8 a type-2 counter, $03B9-$03BF one per slot 1-7) that the stage-2 loader's
;       slot scanner fills in after install with each slot's detected device code; zero-filled
...
; [AI] The 6502's perpetual main loop after the SoftCard handoff: it banks the language card, fires
;       the CPU-switch trigger to run the Z-80, and on yield-back services the Z-80's request before
;       looping. This is what keeps the Apple side alive while CP/M runs on the Z-80.
WARM_BOOT:
...
; [AI] Vector/jump table of $03C0 (WARM_BOOT) pointers and embedded JMP $03C0 instructions, indexed
;       by a dispatcher to re-enter the warm-boot loop from different yield contexts.
JMP_TABLE:
