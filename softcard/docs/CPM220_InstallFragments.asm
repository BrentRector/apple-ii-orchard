...
; [AI] Z-80 BIOS callback code (not 6502) that the Z-80 sees at $134A under the SoftCard bit-12
;       XOR; reads the slot info byte at $F3BB and the Apple keyboard latch at $E0BE to implement
;       console-status/input BIOS services after the CPU switch.
Z80_CALLBACKS:
...
; [AI] Mixed 6502 thunk plus a Z-80 BIOS address/dispatch table; the leading STA $C0BF/RTS is a
;       6502 soft-switch poke, followed by the table of BIOS routine targets the Z-80 console/disk
;       callbacks branch through.
DISPATCH_DATA:
...
; [AI] Per-slot device metadata array written by the stage-2 loader's slot scanner; encodes the
;       detected device code/class for each Apple II slot so the BIOS can route I/O to the right
;       card.
SLOT_DEV_META:
...
; [AI] The 6502 warm-boot routine installed at $03C0 that re-banks the language card, restores
;       registers via the monitor vectors, and fires the SoftCard CPU switch to hand control back to
;       the Z-80 (looping at the end so it never returns to 6502 code).
WARM_BOOT:
...
; [AI] Vector/jump table at $03F0-$03FF holding multiple $03C0 pointers and JMP $03C0 instructions,
;       giving the Z-80 BIOS and reset paths a fixed entry point into the warm-boot routine.
JMP_TABLE:
