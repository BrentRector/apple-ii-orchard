...
| Cold-boot dispatch cases | `[3, 4]` (handlers `$AD60`/`$ACEE`) | `[3, 4, 6]` (handlers `$FE81`/`$FD83`/…) | **VERSION** | `[DIFF]` **tool-verified**: `cpm_pipeline diff <2.20-44k> <2.23-44k>` → `only in B: [6]` — device **6** is the Videx fix (2.20 has no Pascal-1.1 branch; 2.23 adds it). 2.20 handlers are `$AA00`-based (config-aware BIOS selection). |
...
| Generator-page filler | `$E5` (executes as `PUSH HL`) | `FF FF 00 00 / F7 F7 00 00` (RST traps) | **VERSION** | `[RE]` filler hardening: bogus opcodes replaced by RST-trap bytes (the byte delta is real and tool-verified; this is NOT the cause of the 2.20 boot hang, which is the $C700 CPU-switch destroying the $C800 expansion-ROM window claim) |
| BDOS entry / base | `$9C06` expected (manual 44K BDOS `9C00H`); **tool reports `$CC06`** | `$9C06` / `$9C00` | SAME (config) — but **UNRESOLVED** | `[RE]`/`[DIFF]` if the 44K BDOS is at `9C00H` the entry is `$9C06`, = 2.23-44K (so config, not version, and the 56K `$CC06` was config noise). BUT the handoff loader-scan reports `$CC06` for BOTH 2.20 configs — it is **not config-aware** (likely catching a pre-relocation / 56K-oriented plant). Needs verification. |
