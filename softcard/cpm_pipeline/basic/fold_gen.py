...
def generate(write_master=False):
    """Re-derive BASIC.asm from GBASIC.asm + the IFDEF patches (the PROVENANCE chain).

    BASIC.asm has been promoted to the hand-edited master, so this is provenance-only:
    it will NOT overwrite the committed master unless explicitly forced with
    write_master=True (CLI: --write-master), which DISCARDS any direct edits to BASIC.asm.
    Returns the regenerated text either way."""
    text = MASTER_HEADER + asm_path("GBASIC").read_text(encoding="latin-1")
    ...
    if not write_master:
        raise SystemExit(
            "fold_gen is PROVENANCE-ONLY: BASIC.asm is the hand-edited master (see "
            "CPMV220-44K/utilities/PROVENANCE.md). Re-deriving from GBASIC.asm would DISCARD "
            "direct edits. Pass --write-master to force a full re-derive from the .COM bytes.")
    out.write_text(text, encoding="latin-1")
    print(f"wrote {out} (master OVERWRITTEN from the GBASIC.asm provenance chain)")


if __name__ == "__main__":
    import sys
    generate(write_master="--write-master" in sys.argv)
