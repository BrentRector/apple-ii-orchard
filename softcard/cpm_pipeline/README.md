...
annotated source files in `softcard/docs/`, assemble them fresh, place each
...
Mechanically reproduces what `softcard/docs/CPM_Videx_Difference.md`
documents manually.
...
python -m cpm_pipeline decompile-os     ../CPMV223-44K/CPMV223-44K.DSK out_os
python -m cpm_pipeline decompile-file   ../CPMV223-44K/CPMV223-44K.DSK CPM60.COM out_cpm60
python -m cpm_pipeline decompile-disk   ../CPMV223-44K/CPMV223-44K.DSK out          # interactive
python -m cpm_pipeline decompile-disk   ../CPMV223-44K/CPMV223-44K.DSK out --select CPM60.COM --ai
```
...
python -m cpm_pipeline detect ../CPMV223-44K/CPMV223-44K.DSK
...
python -m cpm_pipeline build \
    --reference ../CPMV223-44K/CPMV223-44K.DSK \
...
python -m cpm_pipeline build 220 \
    --reference ../CPMV220/CPMV220-Disk1.po \
    --output ../../build/cpm220_rebuilt.po \
    --verify
# → BYTE-IDENTICAL to ../CPMV220/CPMV220-Disk1.po

# End-to-end: take a disk, produce a complete annotated source tree
python -m cpm_pipeline generate ../CPMV223-44K/CPMV223-44K.DSK ../../build/cpm223_tree
...
python -m cpm_pipeline trace ../CPMV223-44K/CPMV223-44K.DSK
...
What this means: editing a comment in `softcard/docs/CPM223_BIOS.asm` and rerunning
...
| [`assemble.py`](assemble.py) | Wraps ca65/ld65 (6502) and sjasmplus (Z-80) for `softcard/docs/CPM*.asm` files; returns byte content |
...
**`cwd` matters for ca65 `.incbin`.** The `softcard/docs/CPM*.asm` sources use
`.incbin "softcard/cpm-investigation/loader_XXX.bin", offset, size` to pull
