...
REPO_ROOT = Path(__file__).resolve().parent.parent  # softcard/
DOCS = REPO_ROOT / "docs"
SYMBOLS = REPO_ROOT.parent / "shared" / "symbols"  # symbols live in the shared tree
...
if ! command -v ca65 >/dev/null 2>&1; then
    echo "ca65 not on PATH; source shared/toolchain/env.sh first" >&2
    exit 1
fi

# Run the reconstruction (uses the softcard/docs/CPM*.asm sources in the orchard
