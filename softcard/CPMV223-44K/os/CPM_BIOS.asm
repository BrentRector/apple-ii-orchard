...
PROBE_DEVICES_LOOP:
        ...
        CALL SLOT_IO_ADDR                    ; $FA8E  CD 81 FE
...
PROBE_DEVICES_CHK4:
        ...
        DEC A                            ; $FA95  3D
        JR NZ,PROBE_DEVICES_CHK2                 ; $FA96  20 0B
...
PROBE_DEVICES_NEXT:
        ...
        DEC E                            ; $FAAD  1D
        JR NZ,PROBE_DEVICES_LOOP                 ; $FAAE  20 D5
        RET                              ; $FAB0  C9
; ----------------------------------------------------------------------
...
WBOOT:
        ...
        LD HL,SIGNON_PRINT                ; $FACE  21 59 FF
...
PAGEZERO_REBUILD_HOOKS:
        ...
        LD (BOOT_TMPL_IOBYTE),A               ; $FAE4  32 DD FE
        ; clear a self-modified BIOS state cell ($FED8)
        LD (DISK_HSTACT),A                ; $FAE7  32 D8 FE
        ...
        LD HL,BIOS_VECTOR_WBOOT                     ; $FAEF  21 03 FA
...
CONIN_XLATE_LOOP:
        ...
        JP M,CONIN_XLATE_DONE                  ; $FB29  FA 31 FB
        ; raw key == this entry?
        CP C                             ; $FB2C  B9
        ...
        RET Z                            ; $FB2E  C8
        DJNZ CONIN_XLATE_LOOP                  ; $FB2F  10 F4
; ----------------------------------------------------------------------
...
CONOUT_DISPATCH:
        ...
        CP $02                           ; $FB52  FE 02
        JR NZ,CONOUT_FILTER_NOEXPAND                 ; $FB54  20 4B
CONOUT_VIA_VEC_F392:
...
CONIN_DISPATCH_IOBYTE:
        ...
        JR Z,CONIN_VIA_VEC_F38A                  ; $FB64  28 06
        JR NC,DISPATCH_VIA_HL                 ; $FB66  30 07
CONIN_VIA_VEC_F382:
...
LIST:
        ...
        JR Z,CONOUT_VIA_VEC_F392                  ; $FB79  28 DB
...
PUNCH:
        ...
        JR Z,DISPATCH_VIA_HL                  ; $FB8B  28 E2
...
READER:
        ...
        JR C,CONIN_VIA_VEC_F382                  ; $FB98  38 CE
        ; field == $04 -> reuse the CONIN $F38A vector path
        JR Z,CONIN_VIA_VEC_F38A                  ; $FB9A  28 D0
...
CONOUT_FILTER_NOEXPAND:
        ...
        LD HL,BOOT_TMPL_XORA                 ; $FBBA  21 D4 FE
        ; config byte zero -> derive the offset from the running width
        JR Z,COL_OFFSET_FROM_WIDTH                  ; $FBBD  28 0C
        OR A                             ; $FBBF  B7
        JP P,COL_APPLY_OFFSET_RAW                  ; $FBC0  F2 C6 FB
        DEC HL                           ; $FBC3  2B
; ----------------------------------------------------------------------
...
SETDMA:
        ...
        LD (DISK_DMAADR+2),BC            ; $FBF9  ED 43 E1 FE
        RET                              ; $FBFD  C9
...
SETDMA_1:
        ...
        LD E,A                           ; $FC5B  5F
        OR A                             ; $FC5C  B7
        JR NZ,SETDMA_3                 ; $FC5D  20 12
        LD A,($F397)                     ; $FC5F  3A 97 F3
        OR A                             ; $FC62  B7
        JR Z,SETDMA_2                  ; $FC63  28 06
        CP C                             ; $FC65  B9
        JR NZ,SETDMA_2                 ; $FC66  20 03
        LD (HL),$80                      ; $FC68  36 80
        RET                              ; $FC6A  C9
SETDMA_2:
        LD A,$1F                         ; $FC6B  3E 1F
...
SETDMA_4:
        LD A,(HL)                        ; $FC76  7E
        OR A                             ; $FC77  B7
        JR Z,SETDMA_5                  ; $FC78  28 04
        XOR E                            ; $FC7A  AB
        CP C                             ; $FC7B  B9
        JR Z,SETDMA_6                  ; $FC7C  28 05
SETDMA_5:
        DEC HL                           ; $FC7E  2B
        DJNZ SETDMA_4                  ; $FC7F  10 F5
...
SUB_FCE2:
        ...
        LD HL,RPC_TRIGGER                   ; $FCE6  21 45 FB
        PUSH HL                          ; $FCE9  E5
...
SELDSK:
        ...
        LD DE,DISK_SEKDSK                 ; $FE8E  11 D5 FE
        ...
        JR C,SELDSK_BAD_DRIVE                  ; $FE99  38 1D
        LD A,(HL)                        ; $FE9B  7E
        LD (DE),A                        ; $FE9C  12
        INC DE                           ; $FE9D  13
        LD A,C                           ; $FE9E  79
        ...
        LD HL,DPH_TABLE                     ; $FEA0  21 33 FA
        CALL SLOT_IO_SCALE                    ; $FEA3  CD 85 FE
        PUSH HL                          ; $FEA6  E5
        ...
        ; HL,(nn) operand [RE]
        LD (SECTOR_BLOCK_PATCH+1),A             ; $FEB3  32 E3 FE
        POP HL                           ; $FEB6  E1
        RET                              ; $FEB7  C9
; ----------------------------------------------------------------------
...
; ----------------------------------------------------------------------
DISK_SEKDSK:
        LD HL,BIOS_VECTOR                     ; $FED5  21 00 FA
; ----------------------------------------------------------------------
...
SECTOR_BLOCK_PATCH:
        ...
        ; (nn),A operand [RE]
        LD (RPC_TRIGGER_STORE+1),HL             ; $FEE5  22 49 FB
        XOR A                            ; $FEE8  AF
        ...
        ; manual) [RE]
        CP $06                           ; $FEEF  FE 06
        JR NZ,DEV_INSTALL_LIST                ; $FEF1  20 0A
        ...
        SUB $03                          ; $FEF9  D6 03
        JR DEV_INSTALL_PUNCH                   ; $FEFB  18 13
; ----------------------------------------------------------------------
...
DEV_INSTALL_LIST:
        CP $05                           ; $FEFD  FE 05
        JR Z,DEV_INSTALL_READER                 ; $FEFF  28 22
        SUB $03                          ; $FF01  D6 03
        JR C,DEV_INSTALL_READER                 ; $FF03  38 1E
        JR NZ,DEV_INSTALL_PUNCH                ; $FF05  20 09
        LD HL,KBD_STATUS_40COL+1               ; $FF07  21 15 FB
        LD (HL),$BE                      ; $FF0A  36 BE
        INC HL                           ; $FF0C  23
        INC HL                           ; $FF0D  23
        LD (HL),$1F                      ; $FF0E  36 1F
...
DEV_INSTALL_PUNCH:
        PUSH AF                          ; $FF10  F5
        CALL DEV_HANDLER_LOOKUP                    ; $FF11  CD 84 FF
        POP AF                           ; $FF14  F1
        ...
        CALL DEV_HANDLER_LOOKUP_B+1               ; $FF18  CD 7F FF
        LD (CONIN_DISPATCH+1),HL            ; $FF1B  22 37 FB
        LD A,$03                         ; $FF1E  3E 03
        LD (CCP_LAUNCH+1),A              ; $FF20  32 05 FB
; ----------------------------------------------------------------------
...
DEV_INSTALL_READER:
        ...
        SUB $03                          ; $FF26  D6 03
        JR C,INSTALL_DONE_DEVICES                 ; $FF28  38 06
        CALL DEV_HANDLER_LOOKUP                    ; $FF2A  CD 84 FF
...
INSTALL_DONE_DEVICES:
        ...
        SUB $03                          ; $FF33  D6 03
        JR C,SLOT_IO_SCALE_17                 ; $FF35  38 14
        PUSH AF                          ; $FF37  F5
        CALL DEV_HANDLER_LOOKUP                    ; $FF38  CD 84 FF
        LD (L_FE64),HL                   ; $FF3B  22 64 FE
        POP AF                           ; $FF3E  F1
        CP $02                           ; $FF3F  FE 02
        JR Z,SLOT_IO_SCALE_17                 ; $FF41  28 08
        CALL DEV_HANDLER_LOOKUP_B+1               ; $FF43  CD 7F FF
        LD (L_FE6A),HL                   ; $FF46  22 6A FE
        JR SLOT_IO_SCALE_18                   ; $FF49  18 0B
SLOT_IO_SCALE_17:
...
SLOT_IO_SCALE_18:
        ...
        CALL PROBE_DEVICES                    ; $FF56  CD 82 FA
; ----------------------------------------------------------------------
...
SIGNON_PRINT:
        ...
        CALL SIGNON_EMIT                    ; $FF5C  CD 8F FF
        ; HL -> the cold-boot sign-on banner string
        LD HL,SIGNON_BANNER                     ; $FF5F  21 9E FF
; ----------------------------------------------------------------------
...
SIGNON_BANNER_LOOP:
        ...
        JP Z,PAGEZERO_REBUILD_HOOKS                  ; $FF64  CA E3 FA
        PUSH HL                          ; $FF67  E5
        ; emit one banner char via CONOUT
        CALL CONOUT_DISPATCH                    ; $FF68  CD 4C FB
        POP HL                           ; $FF6B  E1
        INC HL                           ; $FF6C  23
        JR SIGNON_BANNER_LOOP                   ; $FF6D  18 F3
SLOT_IO_SCALE_21:
        LD C,$FD                         ; $FF6F  0E FD
        LD (HL),C                        ; $FF71  71
...
DEV_HANDLER_LOOKUP_B:
        ...
        LD IY,SLOT_IO_SCALE_22                ; $FF7E  FD 21 77 FF
        ; join the shared index tail (HL += A*2; load the word)
        JR DEV_HANDLER_LOOKUP_IDX                    ; $FF82  18 03
; ----------------------------------------------------------------------
...
DEV_HANDLER_LOOKUP:
        ...
        LD HL,SLOT_IO_SCALE_21                ; $FF84  21 6F FF
; ----------------------------------------------------------------------
...
SIGNON_EMIT:
        ...
        JP P,SIGNON_EMIT_CHAR                  ; $FF90  F2 9B FF
        PUSH AF                          ; $FF93  F5
        ...
        CALL CONOUT_DISPATCH                    ; $FF97  CD 4C FB
        POP AF                           ; $FF9A  F1
; ----------------------------------------------------------------------
...
SIGNON_EMIT_CHAR:
        ...
        JP CONOUT_DISPATCH                      ; $FF9B  C3 4C FB
; ----------------------------------------------------------------------
