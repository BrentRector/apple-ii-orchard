...
; ../../docs/CPM_Manual_Reconcile_Facts.md. This is the 2.23 / 44K release; manual addresses
; that are 2.20-config-specific (e.g. the $DA00 BIOS load) are NOT cited against this tree,
; while SHARED architecture (config block, Card Type Table, RPC cells, screen functions,
; BDOS/CCP conventions) is.
...
; [AI] System initialization / install routine reached from the SYSIMG_ENTRY jump at $8000: it calls
;       BIOS-area helpers ($FF98/$FF9D) and reads config-block cells to patch BIOS jump vectors
;       (LIST/PUNCH/READER at $FE59/$FE6D/$FE73) before running the install loop and handing off to
;       $FAE3. [DOC S&HD 2-26/2-27] $F3B9 is SLTTYP, the Card Type Table base (slot-1 entry; the
;       entry for slot S is at $F3B8+S), and $F3BA is the slot-2 entry; the SUB $03 / JR C tests the
;       stored Card Type value, skipping the vector install for values below 3 (0=none, 1=unknown,
;       2=Disk II) and installing it for the serial/printer card classes (3=Comms/CCS, 4=High-Speed
;       Serial/Videx, ...). [DOC S&HD 2-14/2-15] $F398 is SSFTAB, the Software Screen Function Table
;       base (function #1, Clear Screen).
SYS_INIT:
...
; [DOC S&HD 2-26/2-27] Branch within the init routine that configures the second optional device
;       vector ($FE6D) from the slot-2 Card Type Table entry. $F3BA is SLTTYP+1 = $F3B8+2, the
;       detected card type for slot 2 (the PUN:/RDR: general-purpose I/O slot); the SUB $03 / JR C
;       borrow skips the install for stored values below 3 (none/unknown/Disk II).
SYS_INIT_1:
...
; [DOC S&HD 2-14/2-15] Convergence point after the device vectors are patched; it calls the main
;       setup at $FA82, fetches a byte from $F398 and programs it via $FFA8, then falls through into
;       the install-table loop. CORRECTION: $F398 is not a "default-drive byte" -- it is SSFTAB, the
;       base of the Software Screen Function Table ($F398 = function #1, Clear Screen; the 9
;       configurable screen functions run $F398..$F3A0).
SYS_INIT_3:
