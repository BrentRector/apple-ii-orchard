...
## CURRENT STATE — 2026-06-18 (newest; older entries retained below)
...
- **Original thread — fact-sheet Section 9 "2.20→2.23 Differences" now DRAFTED** (commit 1841528; `[RE]`/`[DIFF]`-tagged, same-config 44K basis; the 2.20-44K BIOS is located at physical `0x2200`, ORG `$AA00` = the 56K BIOS shifted down `$3000`). Headline version deltas captured: BIOS rebase `$AA00→$FA00`, dispatch case 6 (Videx fix), `$E5`→RST-trap filler, the 11-byte `$Cn0B` probe; the `$CC06→$9C06` BDOS shift shown to be 56K-vs-44K CONFIG noise. **REMAINING tooling:** extract `cpm-investigation/bios_220_44k.bin` + make BIOS lookup/cold-boot trace config-aware (key on the detected BIOS base, not just the variant string) so `cpm_pipeline diff <2.20-44k> <2.23-44k>` auto-traces it; only then are the dispatch/CPU-switch rows fully tool-verified (CPU-switch `$C400` vs `$0E36` is currently UNCONFIRMED for the clean 44K pair). See [[project_reconcile_source_vs_docs]].
