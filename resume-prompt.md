...
- **STILL PENDING (original thread):** fix `cpm_pipeline` to discover the 2.20-**44K** BIOS (at $AA00 — no `bios_220_44k.bin` exists; the variant id maps both 2.20 configs to the 56K `bios_220.bin`) so a clean **2.20-44K vs 2.23-44K** diff yields the device-6 delta; then add a version-only "2.20->2.23 Differences" section to `softcard/docs/CPM_Manual_Reconcile_Facts.md`. See [[project_reconcile_source_vs_docs]].
