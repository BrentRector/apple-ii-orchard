...
class CPU6502:
    ...
    def __init__(self, slot=6):
        ...
        # instruction fetches honor bank-switched memory (e.g. an Apple
        # language card mapped over $D000-$FFFF). Data accesses still go
        # through read()/write(); leave None for a plain flat fetch.
    ...
    def _fetchb(self, addr):
        ...
        Routes through ``fetch_hook`` when one is installed so the host can
        make fetches honor bank-switched memory; otherwise reads the flat
        plane. Data accesses do NOT use this -- they go through read()/write().
        """
        if self.fetch_hook is not None:
            return self.fetch_hook(addr)
        return self.mem[addr]
    ...
    def _addr_zpx(self):
        ...
        return (self._fetchb((self.pc + 1) & 0xFFFF) + self.x) & 0xFF

    def _addr_zpy(self):
        """Zero-page,Y: (operand + Y) wrapped to zero page."""
        return (self._fetchb((self.pc + 1) & 0xFFFF) + self.y) & 0xFF
