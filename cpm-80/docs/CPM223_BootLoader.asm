...
; [AI] Entry point of the sector-0 boot stub; on first call it sets up the P6-PROM sector reader
;       and skew index, then loads the remaining 10 sectors of track 0 and jumps to stage-2 at
;       $1000.
BOOT_STUB:
...
; ----------------------------------------------------------------------------
; [AI] CP/M physical-sector skew table mapping each load iteration to the physical sector to fetch;
;       the interleave minimizes rotational latency for sequential CP/M block reads.
SKEW_TABLE:
...
; [AI] Stage-2 entry at $1000 reached after the stub loads track 0; banks in language-card RAM,
;       stops the disk motor, resets the console drivers and stack, then checks the boot-success
;       signal ($06) before branching to the main path.
STAGE2_ENTRY:
...
; ----------------------------------------------------------------------------
; [AI] Boot-failure path: prints the null-terminated error string at $1192 and drops into the Apple
;       monitor when the boot-success signal was not $06.
BOOT_FAIL:
LDY #$00
...
; [AI] Main loader path after a successful boot signal; runs the three install copy loops that
;       stage data and Z-80 code into low RAM ($0200-$03F0) and zeroes the scan counters.
MAIN_PATH:
...
; [AI] Slot scanner that walks slots 7 down to 1, uses a self-modified $Cn00 access plus a double
;       checksum to detect a real, stable slot ROM, and probes each present card's Pascal firmware
;       signature.
SLOT_SCAN:
...
; [AI] First-visit-per-slot setup: advances the iteration counter and seeds the handoff routine's
;       slot-specific fields ($03C7/$03C8/$03DE/$03DF) with this slot's ROM page and derived offset.
SCAN_INIT_SLOT:
...
; [AI] No-card / empty-slot fallthrough that forces device code 1 (via X=0 then INX) before storing
;       it for this slot.
SCAN_NO_MATCH:
...
; ----------------------------------------------------------------------------
; [AI] Begins the Pascal signature probe with X=4, comparing the slot's $Cn05 and $Cn07 ID bytes
;       against the 4-entry signature tables to classify the card.
SCAN_DO_PROBE:
...
; ----------------------------------------------------------------------------
; [AI] Writes the resolved device code into the per-slot table ($03B9+slot via the $02F8+$Cn alias)
;       and loops to the next lower slot, ending after slot 0 is reached.
STORE_DEV_CODE:
...
; [AI] Post-scan dispatch: requires exactly one slot to have held a real card; if more than one
;       matched it prints the device-selection string at $1173 and drops to the monitor, otherwise
;       it proceeds to handoff staging.
POST_SCAN:
...
; ----------------------------------------------------------------------------
; [AI] Final-install path for the single-device case: copies the 16-byte Z-80 reset-vector staging
;       block to $03EF, then plants the Z-80 cold-boot jump.
DISPATCH_OK:
LDY #$10
LDA $13EF,Y
STA $03EF,Y
...
;             2.23 CP/M BIOS — the BOOT slot of the BIOS jump table at $FA00)
;
...
; ----------------------------------------------------------------------------
; [AI] Stages the CP/M system for the Z-80: preserves the disk I/O routines into LC RAM, overwrites
;       $0A00 with the runtime disk routines, moves the CCP+BDOS+BIOS image into position, patches
;       the Apple RESET/NMI/IRQ vectors, and jumps into the installed warm-boot routine to trigger
;       the CPU switch.
PREP_HANDOFF:
...
; ----------------------------------------------------------------------------
; [AI] Sums all 256 bytes of the current slot's ROM page (returning the checksum in A with a carry
;       count in X); the scanner calls it twice to distinguish a stable real ROM from a varying
;       floating bus on an empty slot.
CKSUM_SLOT:
LDA #$00
...
; ----------------------------------------------------------------------------
; [AI] General page-block mover that copies X*256 bytes from ($52/$53) to ($50/$51); used
;       throughout the handoff staging to relocate the disk routines and CP/M system image into
;       language-card RAM.
PAGE_COPY:
LDY #$00
@byte_loop:
...
; [AI] Source image of the SoftCard warm-boot/handoff routine (copied to runtime $03C0); banks in
;       LC RAM and then JSRs $0E36, whose Z-80 bytes (C3 39 FB = JP $FB39) the SoftCard hardware
;       uses as the trigger to flip the bus from the 6502 to the Z-80.
WARM_BOOT_IMAGE:
