...
DISK_WAIT_XFER:
        ...
        LD L,(HL)                        ; $FCA5  6E
        INC L                            ; $FCA6  2C
        JP Z,DISK_WAIT_XFER              ; $FCA7  CA A4 FC
        ...
        LD A,(HL)                        ; $FCB1  7E
        OR A                             ; $FCB2  B7
        JP Z,CONOUT_VECTOR               ; $FCB3  CA 56 FC
        DEC (HL)                         ; $FCB6  35
        ...
        LD (HL),A                        ; $FCC9  77
        RET                              ; $FCCA  C9
; [AI] Write-side branch of the sector-address builder, selected when the direction flag marks a
...
SECTOR_ADDR_XFER:
        ...
        PUSH AF                          ; $FCE7  F5
        LD B,$07                         ; $FCE8  06 07
        CALL DISK_WAIT_XFER              ; $FCEA  CD A4 FC
        POP AF                           ; $FCED  F1
        LD B,$0A                         ; $FCEE  06 0A
        LD C,A                           ; $FCF0  4F
...
INIT_PASCAL_1_1:
        ...
        LD (HL),$80                      ; $FE68  36 80
        RET                              ; $FE6A  C9
; [AI] Drive-range guard: rejects requested drive codes above the supported maximum before entering
;       the device-table scan.
INIT_PASCAL_1_1_1:
        LD A,$1F                         ; $FE6B  3E 1F
        CP C                             ; $FE6D  B9
        JP C,DISK_WAIT_XFER              ; $FE6E  DA A4 FC
; [AI] Sets up the 9-entry DEVICE_TABLE scan that maps the requested CP/M drive onto a configured
;       Apple/SoftCard device.
INIT_PASCAL_1_1_2:
        LD HL,DEVICE_TABLE_BASE          ; $FE71  21 A0 F3
        LD B,$09                         ; $FE74  06 09
; [AI] Top of the device-table match loop, reading each table entry to compare against the
;       requested drive's device code.
INIT_PASCAL_1_1_3:
        LD A,(HL)                        ; $FE76  7E
; [AI] SETTRK implementation (entry 10) overlapped here as the loop's compare logic;
;       selects/records the requested track for the next transfer.
SETTRK_IMPL:
        OR A                             ; $FE77  B7
...
; [AI] Device-table loop advance: steps to the previous table entry and repeats until all nine
;       entries are checked.
SETTRK_IMPL_1:
        DEC HL                           ; $FE7E  2B
        DJNZ INIT_PASCAL_1_1_3           ; $FE7F  10 F5
; [AI] Keyboard/console init hook called from the slot scan for keyboard-class cards; branches into
;       the shared device-init exit path.
INIT_KEYBOARD:
        JR INIT_KEYBOARD_3               ; $FE81  18 21
; [AI] Device matched: indexes into the matched table row to fetch the device's configuration byte
;       and routes selection accordingly.
INIT_KEYBOARD_1:
        ...
        OR A                             ; $FE88  B7
        LD C,A                           ; $FE89  4F
        JP P,CONST_VECTOR                ; $FE8A  F2 9A FC
INIT_KEYBOARD_2:
        AND $7F                          ; $FE8D  E6 7F
        LD C,A                           ; $FE8F  4F
        PUSH BC                          ; $FE90  C5
...
; [AI] READ implementation (entry 14): reads the currently selected track/sector into the DMA
;       buffer via the 6502 disk engine.
READ_IMPL:
        CP $3A                           ; $FEBD  FE 3A
        RET NC                           ; $FEBF  D0
; [AI] WRITE implementation (entry 15): writes the DMA buffer to the selected track/sector,
;       computing the 6502-side address and remapping the display/character cell.
WRITE_IMPL:
        CP $77                           ; $FEC0  FE 77
...
;       character range ($40-$7F) when writing to the text screen.
WRITE_IMPL_2:
        AND $3F                          ; $FEDC  E6 3F
        OR $40                           ; $FEDE  F6 40
        LD (HL),A                        ; $FEE0  77
