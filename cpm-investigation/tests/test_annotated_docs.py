...
def test_cpm220_bios_annotated_roundtrip():
    _z80_chunk_roundtrip(
        "CPM220_BIOS.asm", "bios_220.bin",
        expected_savebin="build/CPM220_BIOS.bin",
    )


@pytest.mark.skipif(not HAS_SJASMPLUS, reason="sjasmplus not on PATH")
def test_cpm220_systemimage_annotated_roundtrip():
    _z80_chunk_roundtrip(
        "CPM220_SystemImage.asm", "sysimg_220.bin",
        expected_savebin="build/CPM220_SystemImage.bin",
    )


@pytest.mark.skipif(not HAS_CA65, reason="ca65 + ld65 not on PATH")
def test_cpm220_rwts_annotated_roundtrip():
    """Uses .incbin to fill the GCR encode-table region ($0D56-$0E04) and
    the trailing $0E10-$0FFF region. Single KIL instance also converted."""
    _ca65_chunk_roundtrip("CPM220_RWTS.asm", "rwts_220.bin",
                          org=0x0A00, size=0x0600)


@pytest.mark.skipif(not HAS_CA65, reason="ca65 + ld65 not on PATH")
def test_cpm220_installfragments_annotated_roundtrip():
    """Same JMP_TABLE misinterpretation bug at $03F0 as the 2.23 file; fixed."""
    _ca65_chunk_roundtrip("CPM220_InstallFragments.asm",
                          "installfragments_220.bin",
                          org=0x0200, size=0x0200)


@pytest.mark.skipif(not HAS_CA65, reason="ca65 + ld65 not on PATH")
def test_cpm220_bootloader_annotated_roundtrip():
    """The 2.20 stage-2 prose was generated by an early disassembler with
    multiple off-by-one bugs (boot stub decoded from $0800 instead of $0801,
    etc.). Uses .incbin to emit correct bytes; documentation-only prose lines
    are wrapped in .if 0 / .endif so they don't emit anything."""
    _ca65_chunk_roundtrip("CPM220_BootLoader.asm", "loader_220.bin",
                          org=0x0800, size=0x0C00)
