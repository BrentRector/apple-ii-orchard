...
def main():
    ...
    for label, path, interleave in cases:
        ...
        out_path = f'e:/Orchard/cpm-investigation/staging_{label}.bin'
        with open(out_path, 'wb') as f:
            f.write(staging)
        print(f"\n  Wrote {out_path} ({len(staging)} bytes)")

        # Save the system image (CCP+BDOS+BIOS area, lands at Apple $A300-$B9FF)
        out_path = f'e:/Orchard/cpm-investigation/sysimg_{label}.bin'
        with open(out_path, 'wb') as f:
            f.write(sysimg)
        ...
        out_path = f'e:/Orchard/cpm-investigation/newdisk_{label}.bin'
        with open(out_path, 'wb') as f:
            f.write(new_disk)
