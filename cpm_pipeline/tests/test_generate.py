...
@pytest.mark.skipif(not _has("CPMV233.DSK"), reason="CPMV233.DSK missing")
def test_generate_2_23_tree_structure():
    """Generated tree has analysis + source + symbols + build.sh + README."""
    with tempfile.TemporaryDirectory() as tmp:
        out = Path(tmp) / "tree"
        result = generate(REPO_ROOT / "CPMV233.DSK", out)
        # Directory layout
...
def test_generate_2_23_build_script_round_trips():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = reconstruct_disk(
            "223",
            reference_path=REPO_ROOT / "CPMV233.DSK",
            output_path=rebuilt,
            verify=True,
        )
...
def test_overwrite_protection():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with pytest.raises(FileExistsError):
            generate(REPO_ROOT / "CPMV233.DSK", out, overwrite=False)
        # Now with overwrite=True it works (and the stub gets deleted)
        result = generate(REPO_ROOT / "CPMV233.DSK", out, overwrite=True)
