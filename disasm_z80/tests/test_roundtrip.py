...
def test_smoke_roundtrip():
    ...
    src = REPO_ROOT / "tools" / "smoke_z80.bin"
    if not src.exists():
        pytest.skip("tools/smoke_z80.bin missing")
...
def test_cpm_223_bios_roundtrip():
    ...
    src = REPO_ROOT / "cpm-investigation" / "bios_223.bin"
    cpm_syms = REPO_ROOT / "symbols" / "cpm_2_2.json"
    if not src.exists():
        pytest.skip("cpm-investigation/bios_223.bin missing")
