...
class SjasmFormatter:
    ...
    def _render_data_run(self, run):
        ...
        return ([f"        DEFB    {bytes_str:<48} ; ${run.addr:04X}"], set())

    def _render_string(self, run):
        chars = run.metadata["chars"]
        term = run.metadata["terminator"]
        # If all chars are plain ASCII, emit as quoted; otherwise as bytes
        # with an inline ASCII comment (Apple high-bit chars stay raw).
        if all(0x20 <= b <= 0x7E for b in chars):
            try:
                quoted = chars.decode("ascii").replace('"', '\\"')
                return [f'        DEFB    "{quoted}",${term:02X}'
                        f'    ; ${run.addr:04X}  string']
            except UnicodeDecodeError:
                pass
        # Mixed / high-bit: emit as bytes + decoded comment
        bytes_str = ",".join(f"${b:02X}" for b in run.raw)
        decoded = "".join(
            chr(b & 0x7F) if 0x20 <= (b & 0x7F) <= 0x7E else "."
            for b in chars
        )
        return [f'        DEFB    {bytes_str:<48} ; ${run.addr:04X}  "{decoded}"']
