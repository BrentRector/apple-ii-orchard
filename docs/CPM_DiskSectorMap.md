...
> **CORRECTION BANNER (2026-06-11)** — this map predates the SoftCard
> real-address-map correction (`CPM_SoftCard_RealMap_Findings.md`) and
> the mechanism overturn (wiseowl.com Part 13). Row-level sweep PENDING.
> Known systematic errors: (1) "Final Apple address" entries derived
> via the bit-12-XOR model are wrong above $2000; (2) track-2 roles:
> file-sectors 8..13 are the runtime BIOS verbatim (one page each,
> descending, to Z-80 $FA00..$FF00 = Apple $0A00..$0F00), and
> file-sectors 1-6 + 15 are FF/F7-pattern load padding, NOT
> "trap-marker pages awaiting runtime generation"; (3) any role text
> referencing "$FAB8", "$1A00 callbacks", "$0E36 switch", or the
