...
- New module `cpm_pipeline/disk_writer.py` (or similar). Inverse of
  the format-reading code in nibbler.
- CLI tool: `python -m cpm_pipeline build-dsk <chunk-list> <output.dsk>`.
- Update `cpm-investigation/build_dsk.py` to use the new module
...
- New `cpm_pipeline/disk_format.py` that produces a structured
...
- New module `cpm_pipeline/loader_trace.py`.
...
- New module `cpm_pipeline/z80_trace.py`.
...
- New module `cpm_pipeline/handoff.py`.
...
- New module `cpm_pipeline/version_delta.py`.
...
- New module `cpm_pipeline/generate.py`.
...
stage consumes it. (Same pattern as `symbols/*.json`.)
...
The current `symbols/` files are the starting set.
...
`symbols/*.json` schema works for static address → name mappings.
