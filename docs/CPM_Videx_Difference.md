...
1. Running `python -m nibbler disasm` on a 3 KB Apple II memory image
   ...
   $1100, $1200, $1300).
2. Both `CPMV233.DSK` (CP/M 2.23, DOS 3.3 sector order) and
   `CPM220Disk1.po` (CP/M 2.20, ProDOS sector order) yield the
...
| File | Purpose |
|------|---------|
| `cpm-investigation/extract_loader.py` | Builds the 3 KB Apple II memory image |
| `cpm-investigation/loader_223.bin` | 2.23 boot loader image ($0800-$13FF) |
| `cpm-investigation/loader_220.bin` | 2.20 boot loader image ($0800-$13FF) |
| `cpm-investigation/disasm_223_linear.txt` | Full 6502 linear disassembly |
| `cpm-investigation/disasm_220_linear.txt` | Full 6502 linear disassembly |
